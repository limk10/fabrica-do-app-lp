import bannerImage from "common/assets/image/appCreativeTwo/banner_image.webp";
      ...
      <Container width="1400px">
        <BannerContentWrapper>
          <BannerContent>
            ...
            />
            <BannerText>
              <div className="animate__animated animate__fadeInUp">{text}</div>
            </BannerText>
            {button.length > 0 ? (
                ...
                {button?.map((item, index) => {
                  return (
                    <Link href={item?.link} key={index}>
                      <a target="_blank">
                        <Image src={item?.thumb?.src} alt={item?.title} />
                      </a>
                    </Link>
                  );
                })}
